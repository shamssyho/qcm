...
import { fetchQuestionsByQuestionnaire, submitReponse, fetchResultatTotal } from '../../services/api';
...
const QuestionStagiaire: React.FC = () => {
    ...
    const [resultatTotal, setResultatTotal] = useState<number | null>(null);
    ...
    const handleFinish = async () => {
        try {
            const result = await fetchResultatTotal(parseInt(id_questionnaire!));
            setResultatTotal(result.resultatTotal);
            setIsFinished(true);

            const percentage = (result.resultatTotal / questions.length) * 100;
    ...
    if (isFinished && resultatTotal !== null) {
        return (
            <div className="p-5 bg-gray-200 mx-auto my-0 mt-24 rounded-2xl text-gray-800 w-11/12 md:w-2/3">
                <div className="max-w-4xl mx-auto p-4 text-center">
                    ...
                    <p className="text-xl font-bold mt-4">Votre score est de {resultatTotal} sur {questions.length}.</p>
